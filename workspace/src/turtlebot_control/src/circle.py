...
class Node:
    def __init__(self):
        ...
        self.waypoints = [
            np.array([[x], [y]])
            for (x, y) in [
                [3.5 + r * np.cos(t), 3 + r * np.sin(t)]
                for t in np.arange(0, 2 * np.pi, 0.4)
            ]
        ]
        rospy.loginfo("Controller started")
        self.u = np.array([[0.0, 0.0]]).T
        self.continuous = True
        self.out = open("out.json", "w")
    ...
    def update(self, pose, t):
        x = np.array([pose]).T
        n = pose[-1] + np.pi / 2
        x = x - np.array([[0.03 * np.cos(pose[-1]), 0.03 * np.sin(pose[-1]), 0]]).T
        x = x - np.array([[0.045 * np.cos(n), 0.045 * np.sin(n), 0]]).T
        ...
        u[0] = dir * np.abs(u[0])
        u[0] = np.sign(u[0]) * max(min(abs(u[0].item()), 0.2), 0.1)
        ...
        data = json.dumps(
            {
                "t": t.to_time(),
                "closest_theta": int(closest_theta),
                "mu": m,
                "waypoint": self.cur_waypoint,
                "error": e.flatten().tolist(),
                "r": r.flatten().tolist(),
                "x": x.flatten().tolist(),
                "u": u.flatten().tolist(),
                "K": K.flatten().tolist(),
